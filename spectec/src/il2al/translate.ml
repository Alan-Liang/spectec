...
and handle_special_lhs lhs rhs free_ids =
  ...
  | _ when (Il.Print.string_of_typ rhs.note) = "instrstackT" ->
    [captureI { lhs with note = listT instrT } ~at:lhs.at]
...
let translate_context ctx =
  ...
  match ctx.it with
  | Il.CaseE (({it = Atom.Atom id; _} as atom ::_)::_ as case, { it = Il.TupE args; _ }) when List.mem id context_names ->
    let destruct = caseE (case, List.map translate_exp args) ~note:gframeT ~at in
    [
...
and translate_rgroup (rule: rule_def) =
  ...
  let al_params =
    if List.mem instr_name ["frame"; "label"; "handler"; "prompt"] then [] else
    args_of_case winstr
