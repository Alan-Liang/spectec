...
let insert_assert exp =
  let at = exp.at in
  match exp.it with
  | Il.CaseE (({it = Atom id; _}::_)::_, _) when List.mem id context_names ->
...
let translate_context_winstr winstr =
  ...
  [
    letI (destruct, getCurContextE (Some kind) ~note:gframeT) ~at:at;
    insert_assert vals;
  ] @ insert_pop' vals @ [
...
let translate_context ctx =
  ...
  match ctx.it with
  | Il.CaseE (({it = Atom id; _} as atom::_)::_ as case, { it = Il.TupE args; _ }) when List.mem id context_names ->
    let destruct = caseE (case, List.map translate_exp args) ~note:gframeT ~at:at in
    [
      letI (destruct, getCurContextE (Some atom) ~note:gframeT) ~at:at;
    ],
    exitI atom ~at:at
  | Il.CaseE ([atom]::_, _) ->
    [
